...
public class ShahsFragment2 extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        // Inflate the shahs_2.xml layout
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX < 0) {
                        // Swipe left detected, go to ShahsFragment3
                        replaceFragment(new ShahsFragment3());
                    } else {
                        // Swipe right detected, go back to ShahsFragment1
                        replaceFragment(new ShahsFragment1());
                    }
                    return true;
                }
            }
