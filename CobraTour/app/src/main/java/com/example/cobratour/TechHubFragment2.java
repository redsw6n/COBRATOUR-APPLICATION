...
public class TechHubFragment2 extends Fragment {
    ...
    private void setupButtonListeners(View view) {
        ...
        thirdPageButton.setOnClickListener(v -> replaceFragment(new TechHubFragment3(), true));
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX < 0) {
                        // Swipe left detected, go to TechHubFragment3
                        replaceFragment(new TechHubFragment3(), true);
                    } else {
                        // Swipe right detected, go back to TechHubFragment
